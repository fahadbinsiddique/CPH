...
export default function ConsultantCard({ consultant }) {
  ...
  return (
    ...
    <motion.div
      ...
      className="h-full"
    >
      {/* h-full và flex flex-col */}
      <div className="group flex h-full flex-col overflow-hidden rounded-2xl shadow-card transition-all duration-500 hover:shadow-float">
        
        {/* IMAGE CONTAINER  */}
        <div className="relative h-56 w-full shrink-0 overflow-hidden bg-stone-100/80 sm:h-60">
        ...
        <div className="flex flex-1 flex-col p-4 sm:p-5">
          {/* Name & Title */}
          <div>
            <h3 className="line-clamp-1 text-base font-bold text-stone-900 transition-colors group-hover:text-teal-600">
              {user?.full_name}
            </h3>
            ...
            {specializations?.length > 0 ? (
              <>
                {specializations.slice(0, 2).map((s) => (
                  <Badge
                    key={s.id}
                    className="rounded-md border-none bg-stone-50 px-2 py-0.5 text-[10px] font-medium text-stone-600 shadow-none"
                  >
                    {s.name}
                  </Badge>
                ))}
                {specializations.length > 2 && (
                  <Badge className="rounded-md border-none bg-stone-50 px-2 py-0.5 text-[10px] font-medium text-stone-400 shadow-none">
                    +{specializations.length - 2}
                  </Badge>
                )}
              </>
          ...
          <div className="mt-2 flex items-center gap-1.5 border-t border-stone-100 pt-2.5 text-xs font-medium text-stone-500">
          ...
          <div className="mt-2.5 flex items-center justify-between">
            ...
            <span
              className={`text-[10px] font-semibold hidden sm:inline ${
                is_available ? 'text-emerald-600' : 'text-stone-400'
              }`}
            >
          ...
          <div className="mt-auto flex gap-2 pt-4">
            <Link href={`/consultant/${slug}`} className="flex-1">
              <Button
                variant="outline"
                className="h-9 w-full rounded-xl border-stone-200 text-xs font-semibold text-stone-700 transition-colors hover:border-teal-300 hover:bg-stone-50 hover:text-teal-700"
              >
                Profile
              </Button>
            </Link>
            <Button
              ...
              className="group/btn h-9 w-[60%] rounded-xl bg-teal-600 text-xs font-semibold text-white shadow-sm transition-all hover:bg-teal-700 disabled:cursor-not-allowed disabled:bg-stone-200 disabled:text-stone-400"  >
