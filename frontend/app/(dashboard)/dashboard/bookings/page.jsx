...
function AppointmentCard({ appointment, onCancel }) {
  ...
  return (
    <motion.div variants={cardVariants} initial="hidden" animate="visible" exit="exit" layout>
      <Card
        className={`border border-slate-200/60 bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-xl hover:border-teal-500/20 rounded-2xl overflow-hidden transition-all duration-500 group`}
      >
        {/* Top gradient accent line */}
        <div
          className={`h-1 bg-gradient-to-r ${config.bg} from-teal-400 via-emerald-400 to-teal-400 scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left`}
        />
        ...
        <CardContent className="p-5 sm:p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 self-start md:self-center">
              <div
                className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-xl border ${config.color} shadow-sm`}
              >
...
export default function BookingsPage() {
  ...
  const tabs = [
    ...
    { value: 'pending', label: 'Pending', count: filter('pending').length, icon: ClockIcon },
    {
      value: 'confirmed',
      label: 'Confirmed',
      count: filter('confirmed').length,
      icon: CheckCircle2,
    },
    {
      value: 'completed',
      label: 'Completed',
      count: filter('completed').length,
      icon: CheckCircle2,
    },
