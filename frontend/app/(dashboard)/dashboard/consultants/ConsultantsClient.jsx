...
import { verifyConsultant, deleteConsultant, createConsultant, updateConsultant } from '../actions/consultantActions';
...
export default function ConsultantsClient({ initialConsultants = [], initialSpecializations = [] }) {
  ...
  const handleVerify = async (id, value) => {
    setUpdatingId(id);
    try {
      const result = await verifyConsultant(id, value);
      if (result.success) {
        toast.success(value ? 'Consultant verified' : 'Verification revoked');
        setConsultants(prev => prev.map(c => c.id === id ? { ...c, is_verified: value } : c));
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || 'Failed to delete consultant');
        setConfirmDelete(null);
      }
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let result;
      if (editing) {
        result = await updateConsultant(editing.id, fd);
      } else {
        result = await createConsultant(fd);
      }
